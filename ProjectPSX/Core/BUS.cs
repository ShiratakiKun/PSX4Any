...
namespace ProjectPSX
{
    ...
    public class BUS
    {
        ...
        private static string biosFilePath = "./bios/scph1001.bin"; //SCPH1001 //openbios
        ...
        internal unsafe void loadBios()
        {
            try
            {
                byte[] rom = File.ReadAllBytes(biosFilePath);
                Marshal.Copy(rom, 0, (IntPtr)biosPtr, rom.Length);

                Console.ForegroundColor = ConsoleColor.DarkGreen;
                Console.WriteLine("[BUS] BIOS File found: " + biosFilePath);
                Console.WriteLine("[BUS] Contents Loaded.");
                Console.ResetColor();
            }
            catch (Exception e)
            {
