...
namespace ProjectPSX.Devices
{
    ...
    public class SPU
    {
        ...
        internal ushort load(uint addr)
        {
            switch (addr)
            {
                case uint _ when (addr >= 0x1F801C00 && addr <= 0x1F801D7F):
                    ...
                    {
                        case 0x0:
                            return voices[index].volumeLeft.register;
                        case 0x2:
                            return voices[index].volumeRight.register;
                        case 0x4:
                            return voices[index].pitch;
                        case 0x6:
                            return voices[index].startAddress;
                        case 0x8:
                            return voices[index].adsr.lo;
                        case 0xA:
                            return voices[index].adsr.hi;
                        case 0xC:
                            return voices[index].adsrVolume;
                        case 0xE:
                            return voices[index].adpcmRepeatAddress;
                        default:
                            return 0xFFFF;
                    };
        ...
        public bool tick(int cycles)
        {
            ...
            //Clamp sum
            sumLeft = (MathHelper.Clamp(sumLeft, -0x8000, 0x7FFF) * (mainVolumeLeft << 1)) >> 15;
            sumRight = (MathHelper.Clamp(sumRight, -0x8000, 0x7FFF) * (mainVolumeRight << 1)) >> 15;
