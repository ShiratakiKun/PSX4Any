...
namespace ProjectPSX.Devices
{
    ...
    public class CDROM
    {
                ...
                case 0x1F801801:
                    if (INDEX == 0)
                    {
                        if (cdDebug)
                        {
                            ...
                            Console.WriteLine($"[CDROM] [W01.0] [COMMAND] {value:x2} {CollectionExtensions.GetValueOrDefault(commands, value/*"---"*/)}");
                            Console.ResetColor();
                        }
        ...
        private void Cmd_02_SetLoc()
        {
            if (isLidOpen)
            {
                Span<byte> lidOpenResponse = stackalloc byte[] { 0x11, 0x80 };
                responseBuffer.EnqueueRange(lidOpenResponse);
        ...
        private void Cmd_13_GetTN()
        {
            ...
            //Console.ReadLine();
            Span<byte> trackCountResponse = stackalloc byte[] { STAT, 1, DecToBcd((byte)cd.tracks.Count) };
            responseBuffer.EnqueueRange(trackCountResponse);
            interruptQueue.EnqueueDelayedInterrupt(Interrupt.INT3_FIRST_RESPONSE);
        }
        ...
        private void Cmd_14_GetTD()
        {
            ...
            { //returns CD LBA / End of last track
                ...
                Span<byte> trackResponse = stackalloc byte[] { STAT, DecToBcd(mm), DecToBcd(ss) };
                responseBuffer.EnqueueRange(trackResponse);
                //if (cdDebug)
        ...
        private void Cmd_19_Test()
        {
            ...
            switch (command)
            {
                ...
                case 0x05:// 05h      -   INT3(total,success);Stop SCEx reading and get counters
                    ...
                    Span<byte> range = stackalloc byte[] { 0, 0 };
                    responseBuffer.EnqueueRange(range);
                    interruptQueue.EnqueueDelayedInterrupt(Interrupt.INT3_FIRST_RESPONSE);
                    break;
                case 0x20: //INT3(yy,mm,dd,ver) ;Get cdrom BIOS date/version (yy,mm,dd,ver) http://www.psxdev.net/forum/viewtopic.php?f=70&t=557
                    Span<byte> range2 = stackalloc byte[] { 0x94, 0x09, 0x19, 0xC0 };
                    responseBuffer.EnqueueRange(range2);
                    interruptQueue.EnqueueDelayedInterrupt(Interrupt.INT3_FIRST_RESPONSE);
                    break;
                case 0x22: //INT3("for US/AEP") --> Region-free debug version --> accepts unlicensed CDRs
                    Span<byte> range3 = stackalloc byte[] { 0x66, 0x6F, 0x72, 0x20, 0x55, 0x53, 0x2F, 0x41, 0x45, 0x50 };
                    responseBuffer.EnqueueRange(range3);
                    interruptQueue.EnqueueDelayedInterrupt(Interrupt.INT3_FIRST_RESPONSE);
                    break;
        ...
        { //INT5(11h,40h)  ;-Unused/invalid
            Span<byte> vCDResponse = stackalloc byte[] { 0x11, 0x40 };
            responseBuffer.EnqueueRange(vCDResponse);
        ...
        private void applyVolume(byte[] rawSector)
        {
            ...
            for (int i = 0; i < samples.Length; i += 2)
            {
                ...
                samples[i] = (short)MathHelper.Clamp(volumeL, -0x8000, 0x7FFF);
                samples[i + 1] = (short)MathHelper.Clamp(volumeR, -0x8000, 0x7FFF);
            }
        }
